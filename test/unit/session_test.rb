...
class SessionTest < ActiveSupport::TestCase
  context "Sessions" do
    ...
    context "Sessions being tested for participation" do
      setup do
        @session_with_participants = FactoryGirl.create(:future_session, :experiment => @e1, session_participations_count: 1)
        @session_without_participants = FactoryGirl.create(:future_session, :experiment => @e1)
      end

      should "return correct state when it has participants" do
        assert @session_with_participants.has_participants?
      end

      should "return correct state when it does not have participants" do
        assert !@session_without_participants.has_participants?
      end
    end
    ...
    should "return correct start date and end date" do
      assert_equal "01.01.2011", @session.start_date
      assert_equal 120, @session.duration

      @session.start_at = nil
      assert_equal nil, @session.start_date
